...
void D_DoomMain (void)
{
    ...
    TRACE(0x21); // Milestone 0x21: Entering Z_Init
    Z_Init ();
    TRACE(0x22); // Milestone 0x22: Z_Init Complete
    ...
    TRACE(0x23); // Milestone 0x23: Loading Defaults
    ...
    // Find main IWAD file and load it (Directly mapped to 0x81000000 memory WAD)
    TRACE(0x24); // Milestone 0x24: Assigning memory IWAD
    iwadfile = "doom1.wad";
    gamemission = doom;

    modifiedgame = false;

    TRACE(0x25); // Milestone 0x25: Adding WAD file
    DEH_printf("W_Init: Init WADfiles.\n");
    D_AddFile(iwadfile);
    TRACE(0x26); // Milestone 0x26: WAD file added
#if ORIGCODE
    ...
    TRACE(0x27); // Milestone 0x27: Identifying version
    D_IdentifyVersion();
    InitGameVersion();

    TRACE(0x28); // Milestone 0x28: Generating Hash Table
    W_GenerateHashTable();
    TRACE(0x29); // Milestone 0x29: Hash Table Complete
...
//    W_PrintDirectory();
    ...
    TRACE(0x30); // Milestone 0x30: Entering M_Init
    DEH_printf("M_Init: Init miscellaneous info.\n");
    M_Init ();

    TRACE(0x40); // Milestone 0x40: Entering R_Init
    DEH_printf("R_Init: Init DOOM refresh daemon - ");
    R_Init ();
    TRACE(0x4F); // Milestone 0x4F: R_Init Complete!

    TRACE(0x50); // Milestone 0x50: Entering P_Init
    DEH_printf("\nP_Init: Init Playloop state.\n");
    P_Init ();

    TRACE(0x51); // Milestone 0x51: Setting up sound
    DEH_printf("S_Init: Setting up sound.\n");
    S_Init (sfxVolume * 8, musicVolume * 8);

    TRACE(0x52); // Milestone 0x52: Checking Net Game
    DEH_printf("D_CheckNetGame: Checking network game status.\n");
    D_CheckNetGame ();

    PrintGameVersion();

    TRACE(0x53); // Milestone 0x53: Setting up HUD
    DEH_printf("HU_Init: Setting up heads up display.\n");
    HU_Init ();

    TRACE(0x54); // Milestone 0x54: Init Status Bar
    ...
    if (gameaction != ga_loadgame )
    {
		if (autostart || netgame)
            ...
            TRACE(0x60); // Milestone 0x60: Starting Title
			D_StartTitle ();                // start up intro loop
        }
    }

    TRACE(0x70); // Milestone 0x70: Entering Main Game Loop D_DoomLoop!
    D_DoomLoop ();
}
