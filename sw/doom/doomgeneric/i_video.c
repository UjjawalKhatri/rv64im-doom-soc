...
extern pixel_t static_screen_buffer[];
byte *I_VideoBuffer = (byte *)static_screen_buffer;
...
void I_InitGraphics (void)
{
    ...
    /* Use static screen buffer directly for zero-copy rendering */
	I_VideoBuffer = (byte*)static_screen_buffer;
	screenvisible = true;
...
void I_SetPalette (byte* palette)
{
    ...
    // ------------------------------------------------------------------
    // Push the gamma-corrected palette into the SoC's hardware palette RAM.
    // Window: 0x2001_0000 + index*8, one 64-bit store per entry (0x00RRGGBB).
    // This is what gives us DOOM's real palette effects: the red damage
    // flash, the gold item-pickup flash, and the green radiation-suit tint
    // (driven by ST_doPaletteStuff), plus live gamma correction.
    // ------------------------------------------------------------------
    {
        volatile uint64_t *hw_palette = (volatile uint64_t *)0x20010000ULL;
        for (i = 0; i < 256; ++i) {
