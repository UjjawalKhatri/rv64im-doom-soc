...
#define DOOM_ZONE_BASE 0x82000000ULL
#define DOOM_ZONE_SIZE (16 * 1024 * 1024)

byte *I_ZoneBase (int *size)
{
    *size = DOOM_ZONE_SIZE;
    return (byte *)DOOM_ZONE_BASE;
}
...
void I_Error (char *error, ...)
{
    ...
    // Milestone: Fatal error! Turn on LEDs = 0xEE
    *(volatile uint64_t *)0x10001008ULL = 0xEEULL;

    // Format the full error message into a local buffer.
    ...
    // -------------------------------------------------------------------
    // BARE-METAL FATAL ERROR REVEAL
    // The default vga_text foreground (palette index 4 = 0x0B0B0B) is
    // near-black and invisible on a black screen, which is why fatal
    // errors previously looked like a "solid black monitor". Force a
    // bright-white-on-black console AND mirror the message to the UART
    // (115200 8N1) so it can be captured over serial as well.
    // -------------------------------------------------------------------
    extern void vga_text_init(unsigned char bg_color, unsigned char fg_color);
    extern void vga_text_set_color(unsigned char fg, unsigned char bg);
    extern void vga_text_puts(const char *str);

    // Reliable channel: UART serial (open a terminal at 115200 8N1).
    printf("\n\n*** DOOM FATAL ERROR (I_Error) ***\n%s\n", msgbuf);

    // Visible channel: bright white text on the VGA monitor.
    vga_text_init(0x00, 0x80);   // bg=black(0), fg=white(index 128 = 0xFFFFFF)
    vga_text_set_color(0x80, 0x00);
    vga_text_puts("\n  ========================================\n");
    vga_text_puts("  >>>        DOOM FATAL ERROR         <<<\n");
    vga_text_puts("  ========================================\n\n  ");
    vga_text_puts(msgbuf);
    vga_text_puts("\n\n  System halted.\n");

    // Halt here so no secondary fault (atexit handlers, exit()) can wipe
